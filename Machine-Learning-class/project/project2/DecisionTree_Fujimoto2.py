...
class DecisionTree:
    def __init__(self, X, y, max_depth= 2, min_leaf_size= 1,depth= 0,classes= None):
        ...
        class_ratio = self.class_count / self.N
        self.gini = 1 - np.sum(class_ratio **2)
        ...
        for num in range(self.X.shape[1]):
            col_values = self.X[:, num].copy()
            col_values = np.sort(col_values)
                
            for i in range (len(col_values)):
                sel = self.X[:,num] <= col_values[i]
                n_left = np.sum(sel)
                n_right = np.sum(~sel)
                ...
                if (n_left >= min_leaf_size) & (n_right >= min_leaf_size):
                    ...
                    if (gini <= best_gini):
                        best_gini = gini
                        self.axis = num
                        if((i +1)== len(col_values)):
                            self.t = col_values[i]
                        else:
                            self.t = (col_values[i]+col_values[i+1])/2
    ...
    def score(self,X,y):
        ...
        accuracy = (np.sum(self.y== y_pred))/ self.N   
        return accuracy
